...
with uproot.open(file_path) as file:
    ...
    selection = ak.sum((data["Muon_pt"] > 5) & (data["Muon_tightId"] == 1), axis=1) >=2 # this should be >=2
    selected_data = data[selection]
    
    #obj_selection = ((selected_data["Muon_pt"] > 5) & (selected_data["Muon_mediumPromptId"] == 1) )
    obj_selection = ((selected_data["Muon_pt"] > 5) & (selected_data["Muon_tightId"] == 1) )
    ...
    pair_combinations = ak.combinations(t_vec, 2, axis=1)
    idx_pair_combinations = ak.argcombinations(t_vec, 2, axis=1)
    print("How many combinations of 2?")
    print(ak.num(pair_combinations,axis=1))

    
    start_time = time.time()
    inv_mass = (pair_combinations["0"] + pair_combinations["1"]).mass
    end_time = time.time()
    print("Invariant mass:", inv_mass)
    print(f"Time to compute invariant masses: {end_time - start_time:.4f} seconds")
    ...
    start_time = time.time()
    inv_mass_valid = (valid_pairs["0"] + valid_pairs["1"]).mass
    end_time = time.time()
    ...
    print("diff")
    print(ak.min(np.abs(inv_mass - 91), axis=1))
    min_inv_mass = ak.min(np.abs(inv_mass - 91), axis=1, keepdims = True)
    best_pair = ak.argmin(np.abs(inv_mass - 91), axis=1, keepdims = True)
    print(f"  invariant mass: ",inv_mass)
